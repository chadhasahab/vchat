...
$usrObj->updateSession();
$user = $usrObj->userdata();
if (isset($_GET['username']) && !empty($_GET['username'])) {
    $profileData = $usrObj->getUserByUsername($_GET['username']);
    if (!$profileData) {
        $usrObj->redirect('home.php');
...
<head>
    ...
    <script type="text/javascript">
        const conn = new WebSocket('ws://localhost:8080/?token=<?= $usrObj->sessionID ?>')
    </script>
</head>
